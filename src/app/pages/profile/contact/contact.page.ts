...
export class ContactPage implements OnInit, OnDestroy {
  ...
  getContactData() {
    this.emailObs = this.profileService.getEmail(null, 20);
    this.subs.sink = this.emailObs.subscribe(emails => {
      this.emails = emails;
      this.primaryEmail = this.emails.find(x => x.flag_primary);
    });

    this.phoneObs = this.profileService.getPhone(null, 20);
    this.subs.sink = this.phoneObs.subscribe(phones => {
      this.phones = phones;
  ...
  confirmEmailToken(token) {
    this.subs.sink = this.profileService.confirmEmailHash(token).subscribe(res => {
      console.log({res});
      this.emailObs = this.profileService.getUserProfile();
    });
  }
