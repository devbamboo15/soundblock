...
export class ContractPage implements OnInit, OnDestroy {
  ...
  getContractData() {
    this.contractObs = combineLatest(
      this.projectService.getProjectContract(this.projectId),
      this.projectService.getProjectService(this.projectId)
    );
    this.subs.sink = this.contractObs.subscribe(([contract, service]) => {
      console.log({contract, service});
      this.checkContractPermission(service.service_uuid);
      this.contract = contract;
      if (!contract.users) return;
  ...
  async modifyContract(contract) {
    ...
    modal.onDidDismiss().then(result => {
      if (result.data) {
        this.contractObs = this.projectService.updateProjectContract(this.projectId, result.data.data);
      }
    });
