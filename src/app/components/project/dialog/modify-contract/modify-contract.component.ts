...
export class ModifyContractComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    const inviteMembers = _.cloneDeep(this.contract.contract_invites.filter(user => user.user_payout).map(user => {
      return {name: user.invite_name, email: user.invite_email, role: user.invite_role, payout: user.user_payout};
    }));
